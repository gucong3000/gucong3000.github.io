...
*/
( function($, undefined) {
var scrPath = $(document.scripts).last().data("path");
...
function cssSet(elem){
    ...
    if (window.PIE) {
		...
		$.getScript(scrPath + "PIE.js", runcode)
	}
}
...
if($.browser.msie < 9){
	...
	$.cssHooks.background = {
		get: function( elem ) {
			with (elem.style){
				return PieBackground || background;
			}
		},
		set: function( elem, value ) {
			if( elem.style.PieBackground || /linear-gradient/i.test(value) || /\,/.test(value) || ( $.browser.msie < 7 || /.png/i.test(value) )){
